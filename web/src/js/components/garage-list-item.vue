  ...
  <div class="garage-item-grid">
    <div class="garage-name">
      ...
      <template v-else>
        ...
        <button
          ...
          @click="
            editing = !editing;
            Object.assign(garage, updated_garage);
          "
        >
          Cancel
        </button>
    ...
    <div v-if="editing" class="edit-garage">
      <garage-form
        ...
        @change="
          editing = false;
          Object.assign(updated_garage, garage);
        "
      ></garage-form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    deleteGarage() {
      // this.$emit('change', this.garage.id);
      $.ajax({
        type: 'DELETE',
        contentType: 'application/json',
        url: `/garages/`,
  ...
  watch: {
    garage(g) {
      console.log('garage update:' + JSON.stringify(g));
